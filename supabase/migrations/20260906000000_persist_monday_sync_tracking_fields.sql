-- ============================================================================
-- Persist Monday.com sync tracking fields (_mondayId, _mondayAttachLocked,
-- _mondayAssetId) through the normalized app_state tables
-- ============================================================================
-- save_app_state()/load_app_state() normalize tasks and attachments into
-- fixed-column tables, and these three ad-hoc fields were never added to
-- either the column lists or the two RPC functions. Every save-then-reload
-- round trip silently dropped them:
--   - tasks.t._mondayId       -> used to exact-match a card back to the
--                                Monday.com item it was created from
--   - tasks.t._mondayAttachLocked -> "Lock Files" flag that should stop
--                                attachment sync from touching a card
--   - attachments.a._mondayAssetId -> used to recognize a Monday.com asset
--                                that was already downloaded and uploaded to
--                                Drive, so a repeat sync reuses it instead of
--                                re-uploading it
-- Without persistence, every attachment sync re-uploaded every file from
-- scratch (the "already uploaded" check never found a match after a reload),
-- and exact-ID card matching never worked at all.
--
-- Run this after 20260808000000 - 20260808000004.
-- Safe to run more than once.
-- ============================================================================

alter table tasks add column if not exists monday_id text;
alter table tasks add column if not exists monday_attach_locked boolean not null default false;
alter table attachments add column if not exists monday_asset_id text;
