-- ============================================================================
-- Fix unqualified DELETEs in save_app_state()
-- ============================================================================
-- Supabase blocks any DELETE with no WHERE clause at all (its built-in
-- guard against accidental full-table deletes), which is exactly what the
-- previous migration used for notif_prefs, push_subscriptions, and
-- password_reset_tokens (those three are always fully replaced on every
-- save, so they were written as unconditional deletes). That's what was
-- actually causing "DELETE requires a WHERE clause" / error 21000 on every
-- save — the earlier duplicate-id theory was a red herring; the dedup
-- hardening from the previous migration is still correct and worth keeping,
-- it just wasn't the live problem.
--
-- Run this AFTER 20260808000000, 20260808000001, and 20260808000002.
